...
export function handlePaused(event: PausedEvent): void {
  const id = Bytes.fromUTF8("Diamond" + event.params.account.toString());
...
export function handleStaked(event: StakedEvent): void {
  const id = Bytes.fromUTF8("Diamond" + event.params.user.toString());
  const booster = Booster.load(id);
...
export function handleTransferSingle(event: TransferSingleEvent): void {
  const id = Bytes.fromUTF8("Diamond" + event.params.operator.toString());
  const booster = Booster.load(id);
